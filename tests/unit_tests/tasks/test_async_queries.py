...
def test_execute_chart_query_fails_loudly_when_result_not_cached(
    ...
    Regression for the async resubmit loop: when GLOBAL_ASYNC_QUERIES is on and a
    result exceeds ``DATA_CACHE_MAX_VALUE_SIZE``, the write is skipped but the query
    still succeeds. Async delivery relies on the client reading the result back from
    the DATA cache, so a missing entry makes the endpoint's ``force_cached`` read
    raise ``CacheLoadError`` and reschedule this task — forever.

    Pre-fix (RED): the task reported the cache key and returned success even though
    nothing was cached, so the client resubmitted endlessly. Post-fix (GREEN): the
    task raises so the client sees a clear terminal error, and it never publishes a
